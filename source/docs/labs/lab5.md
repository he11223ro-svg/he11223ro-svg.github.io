...
Оптимизация гиперпараметров ансамблевого алгоритма **Random Forest Regressor** (увеличение числа деревьев `n_estimators=150` и ограничение глубины `max_depth=15`) позволила добиться наименьшей среднеквадратичной ошибки (**RMSE = 0.453**) и высокого коэффициента детерминации (**R2 = 0.844**), что превосходит базовые линейные модели.
...
def predict_price(features: list):
    ...
    return {"estimated_price": float(predicted_price[0])}
